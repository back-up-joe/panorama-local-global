...
def _extraer_fecha_autor(driver):
    ...
    try:
        # Versión corregida basada en el HTML proporcionado
        ...
        try:
            # Método alternativo para autor: buscar por itemprop='author'
            autor = driver.find_element(By.CSS_SELECTOR, "[itemprop='author'] a").text
        except:
            try:
                # Método alternativo: buscar cualquier enlace dentro del contenedor de autor
                autor = driver.find_element(By.CSS_SELECTOR, ".edgtf-post-info-author a").text
            except:
                try:
                    # Último intento: buscar texto "Por" y tomar el siguiente enlace
                    autor_element = driver.find_element(By.XPATH, "//div[contains(@class, 'edgtf-post-info-author')]")
                    autor = autor_element.find_element(By.TAG_NAME, "a").text
                except:
                    pass
        
        try:
            # Método alternativo para fecha: buscar por itemprop='dateCreated'
            fecha_element = driver.find_element(By.CSS_SELECTOR, "[itemprop='dateCreated']")
            fecha_link = fecha_element.find_element(By.CSS_SELECTOR, "a")
            fecha_raw = fecha_link.text
        except:
            try:
                # Buscar fecha por clase
                fecha_element = driver.find_element(By.CSS_SELECTOR, ".edgtf-post-info-date")
                fecha_raw = fecha_element.text
            except:
                try:
                    # Buscar en meta tags para fecha
                    fecha_raw = driver.find_element(By.CSS_SELECTOR, "meta[property='article:published_time']").get_attribute("content")
                except:
                    try:
                        # Último intento: buscar cualquier texto que parezca fecha
                        fecha_element = driver.find_element(By.XPATH, "//div[contains(@class, 'edgtf-post-info-date')]")
                        fecha_raw = fecha_element.text
                    except:
                        pass
    
    # Solo parsear la fecha si se encontró
    if fecha_raw:
